...
$head .=" <form action='index.php' method='get'><input size=1 hidden name=sort id=sort value=".$sort."></input>";
echo $head;
//if (!$srok) $srok=7;
...
for ($tmpcnt=0;$tmpcnt<$counter;$tmpcnt++){
  $sql="SELECT SUM(itogo), SUM(balans), SUM(pro60), SUM(pro40), SUM(gotivka) FROM uber WHERE uber.iduber='".$row[$tmpcnt][6]."' AND (start BETWEEN '$start 00:00:00' AND '$end 23:59:59') ;";
//echo $sql;
//echo "\n";
  $result = mysqli_query($dbh,$sql);// or die('query error: ' . mysql_error());
...
for ($tmpcnt=0;$tmpcnt<$counter;$tmpcnt++){
  ...
  $sql="SELECT SUM(itogo), SUM(balans), SUM(pro60), SUM(pro40), SUM(gotivka)  FROM uklon WHERE ((uklon.pozivnoy='".$row[$tmpcnt][8]."'".") OR (uklon.pozivnoy='".$row[$tmpcnt][9]."')) AND (start BETWEEN '$start 00:00:00' AND '$end 23:59:59')";
...
////bolt
for ($tmpcnt=0;$tmpcnt<$counter;$tmpcnt++){
  $sql='SELECT SUM(itogo), SUM(balans), SUM(pro60), SUM(pro40), SUM(gotivka)  FROM bolt WHERE bolt.telbolt="'.$row[$tmpcnt][7].'"'."AND (start BETWEEN '$start 00:00:00' AND '$end 23:59:59');";
//echo $sql;
//echo "\n";
...
//$sort2=16;
//if ($sort2) {	
//    for ($tmpcnt=0;$tmpcnt<$counter;$tmpcnt++){  //sort
//  }
//}

for ($tmpcnt=0;$tmpcnt<$counter;$tmpcnt++){  //loop table...
  $tmp=$row[$tmpcnt][4];
  if ($bf > 0) {
  ...
  if ($itogtmp){
    ...
    echo "  <td>$idl</td>
    <td>".$row[$tmpcnt][12]."</td> 
    <td><a target='podrobno' href='podrobno.php?uid=$idl&stime=$start&dtime=$end'>".$row[$tmpcnt][11]."</a></td> 
    <td align=left><a target='podrobno' href='podrobno.php?uid=$idl&stime=$start&dtime=$end'>".$row[$tmpcnt][1]."</a></td>
    <td align=left><a target='podrobno' href='podrobno.php?uid=$idl&stime=$start&dtime=$end'>".$row[$tmpcnt][2]."</a></td>
