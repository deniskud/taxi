...
socket.on('connect',function(data){
    ...
    socket.emit("messages4Users",{id:0});
});
...
$(document).ready(function () {
    ...
    $('#sidebar nav button').click(function () {
        if(!$(this).hasClass('active')) {
            ...
            if(_t.attr('data-target') === 'car-park') {
                //--------- Reload park/drivers timeline
                setTimeout(function () {
                    $('#park-full').Timeline('reload');
                },250);
            }
        }
    });
    ...
    // socket.on('carData',(data)=>{consolelog('cardata:'+data);});
    ...
    $('#mess-submit').click(function () {
        if($('#mess-input').val().length > 0) {
            ...
            _m = _m.trim();
            // socket.emit("messageSend",{'user':userId, 'time':getFormattedCurrentSavedDate(new Date()), 'message':_m, 'forUsers':$('#mess-users option:selected').attr('data-user'), 'forUsersGroup':$('#mess-users option:selected').attr('data-user-group')});
    ...
    socket.on('messages4Users',(data)=>{console.log('4users:'+data)})
    ...
    socket.on('messagesUsers',(data)=>{
        ...
        /*by zlodey end*/
        $(data).each(function (i) {
            // $('#mess-users').append('<option data-user-group="' + this[2] + '" data-user="' + this[0] + '"' + (i === 0 && ' selected') + '>' + this[1] + '</option>');
            $('#mess-users').append('<option data-user="' + this.id + '"' + (i === 0 && ' selected') + '>' +  this.username + '</option>');
        });
    });
    ...
    socket.on('CheckNewMessageForUser',(data)=>{
        ...
        /*by zlodey end*/
        if(parseInt(data.id) === userId || parseInt(data.id) === 0) {
    ...
    //--------- Finance start date select
    $('#park-start-date').val(getFormattedDatepickerDate(-1)).datepicker({
        format: "dd/mm/yyyy",
        weekStart: 1,
        language: "ru",
        todayHighlight: true,
        ...
        $('#park-week-btn').removeAttr('disabled');
        $('#park-end-date').datepicker('setStartDate', $(this).datepicker('getDate'));
        // TODO request send + loader
    });

    //--------- Finance end date select
    $('#park-end-date').val(getFormattedDatepickerDate(0)).datepicker({
        format: "dd/mm/yyyy",
        weekStart: 1,
        language: "ru",
        todayHighlight: true,
        startDate: getFormattedDatepickerDate(-1),
        endDate: getFormattedDatepickerDate(0)
    }).change(function () {
        $('#park-week-btn').removeAttr('disabled');
        $('#park-start-date').datepicker('setEndDate', $(this).datepicker('getDate'));
        // TODO request send + loader
    });

    $('#park-full').Timeline({
        startDatetime: "2021-04-01 00:00",
        endDatetime: "2021-04-02 00:00",
        firstDayOfWeek: 1,
        hideScrollbar:true,
        type: "bar",
        scale: "hour",
        ...
        ruler: {
            ...
            top: {
                ...
                locale:     "ru-RU",
                format:     {
                    timeZone: "UTC", hour12: false, weekday: "long", hour: "fulltime"
                }
            }
        },
        effects: {
        ...
        sidebar: {
            sticky:  true,
            overlay: true,
            list: ["<label>AE3745MC</label>", "<label>BI6543EE</label>"]
        },
        eventData: [
            { start: '2021-04-01 07:30', end: '2021-04-01 12:30', row: 1},
            { start: '2021-04-01 14:30', end: '2021-04-01 20:30', row: 1},
            { start: '2021-04-01 11:00', end: '2021-04-01 16:00', row: 2}
        ]
    });
    ...
    // $('#park-full').Timeline('addEvent', [
    //     {id:21,start:'2021-04-01 07:30',end:'2021-04-01 12:30',row:2,label:'Add Event',content:'test',extend:{toggle:'modal',target:'#myModal'},relation:{after:22,curve:1}},
    //     {id:22,start:'2021-04-01 14:30',end:'2021-04-01 20:30',row:3,label:'Add Event 2',content:'test 2',extend:{toggle:'popover',placement:'top',trigger:'hover'}}]
    // );
...
function drawCars() {
    ...
    // socket.emit("carData",{'cars':['24','27','30'],'time':mainTailsLength});
    socket.emit("carData",{'cars':getSelectedCars(),'time':mainTailsLength});
    ...
    $(data).each(function () {
        ...
        L.circleMarker(_t.lastCoords[0], {
            ...
            L.popup({
                ...
                '<li class="map-popup-list-speed"><span>Скорость авто:</span> <strong>' + _t.speed + '</strong></li>' +
                '<li class="map-popup-list-work-time"><span>Время на линии:</span> <strong>' + _t.workTime + '</strong></li>' +
                '<li class="map-popup-list-coords"><span>Координаты:</span> <strong>' + _t.lastCoords[0][0] + ', ' + _t.lastCoords[0][1] + '</strong></li>' +
                '</ul>'
            ).openOn(mainMap);
...
function carsMovement() {
    ...
    carsMovementTimeout = setInterval(function () {
        socket.emit("carDataMove",{'cars':getSelectedCars(),'time':mainTailsLength});
    }, 1000);
